  ...
  router.post("/", (req, res) => {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .send(
          `One of the fields is blank. Please <a href='/login'>try again</a>`
        );
    }

    //Checks to see if email in DB
    getUserWithEmail(email, db).then((result) => {
      //If email not in DB, tell them to retry login
      if (!result) {
        return res
          .status(400)
          .send(
            `Email has not been registered. Please <a href='/login'>try again</a>`
          );
      }
      ...
      if (result.password != password) {
        return res
          .status(400)
          .send(
            `Password does not match our records. Please <a href='/login'>try again</a>`
          );
      }
